...
with open('test_IO.csv', mode='a') as save_file:
    csv_writer = csv.writer(save_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
    csv_writer.writerow(['timestamp', 'mission', 'persona', 'thing','input', 'output','url'])
 
for eachkey in in_type2name:
    input_name=in_type2name[eachkey]
    output_name='iconHappy'
    urlis=genURL(input_name,output_name) 
...
for otherkey in out_type2name:
    input_name='buttonPress'
    output_name=out_type2name[otherkey]
    urlis=genURL(input_name,output_name)
